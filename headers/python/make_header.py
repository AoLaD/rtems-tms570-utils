...
def makeFile(hout, loaded_jason ):
    ...
    hout.write("#ifndef LIBBSP_ARM_"+loaded_jason.name.upper()+'_'+loaded_jason.peripherals[0].name+'\n')
    hout.write("#define LIBBSP_ARM_"+loaded_jason.name.upper()+'_'+loaded_jason.peripherals[0].name+'\n')
    hout.write('\n')
...
def makeFirstLine(reg,prefix):
    block = prefix+'_'+reg.name
    if(reg.group_name_actual != None):
        #use x insted of reg.group_name_actual
...
loadedJson = json.loads(jason, object_hook=object_decoder)
...
makeFile(hout, loadedJson)
hout.write('\n')
makeStruct(hout, loadedJson)
hout.write('\n')
loadedJson = json.loads(jason, object_hook=object_decoder)
makeRegs(hout, loadedJson)
hout.write('\n')
hout.write("#endif /* LIBBSP_ARM_"+loadedJson.name.upper()+'_'+loadedJson.peripherals[0].name+' */\n')
