...
def printInclude(hout,prefix,file):
    hout.write('#include <bsp/'+prefix+file+'.h>'+"\n")
...
files = ""
licence = ""
hhelp = 'test.py -d <inputDir> -f <filesNames separated by space> -o <outputfile>' 

try:
      opts, args = getopt.getopt(sys.argv[1:],"hd:f:l:o:")
...
for opt, arg in opts:
      if opt == '-h':
         ...
         outputfile = arg
if(hdir == "" or files == "" or licence == "" or outputfile == ""):
    print ('Invalid argument' +hhelp)
    sys.exit()

hout = codecs.open(outputfile, "w", "utf-8")
log = codecs.open(outputfile+"_log", "w", "utf-8")
...
for file in files:
    printInclude(hout,prefix,file.lower())
    log.write("include_bsp_ti_hercules_HEADERS += include/"+prefix+file.lower()+".h\n")
hout.write("\n")
for file in files:
